 ...
 */
class ReindeerSw_DnbBanklink_Helper_Mac extends Mage_Core_Helper_Data {
    static $FieldNames = array(
            '2001' => array('VK_SERVICE', 'VK_VERSION', 'VK_SND_ID', 'VK_STAMP', 'VK_AMOUNT', 'VK_CURR', 'VK_ACC', 'VK_PANK', 'VK_NAME', 'VK_REF', 'VK_MSG'),
    ...
    public function Compose($fields) {
        $mac = "";
        
        foreach(self::$FieldNames[$fields['VK_SERVICE']] as $name)
            $mac .= str_pad(strlen($fields[$name]), 3, '0', STR_PAD_LEFT) . $fields[$name];
    ...
    public function Sign($data) {
        ...
        $config = Mage::getModel('reindeersw_dnbbanklink/config');
